...
**Static HTML site mirrors** (Donovan VGA Planets, Planets.nu help, and similar) from **`scripts/python/static_html_site_export.py`** (console: **`static-html-site-export`**; Donovan-only alias **`donovansvgap-static-export`**) live under agreed **`raw/<corpus>/`** roots: URL-path folders with **`body.html`** (page body) and **`page.meta.yaml`** (JSON metadata including **`source_url`**). Layout and citation habits: **`wiki/meta/corpus-layout-donovansvgap-static-export.md`** (Donovan); add a sibling **`wiki/meta/corpus-layout-*.md`** when a new mirrored host uses the same on-disk shape. For **donovansvgap.com**, **`/museum/`** is skipped by default unless you override exclusions.

### Synthesis strategy: **entity-centric**

When ingesting a large corpus (especially an exported wiki or a **bulk HTML mirror**), **prefer synthesized pages keyed to real-world or domain “things”**—factions, mechanics, ships, places, products, organizations—using `**type: entity`** (or `**concept**` for themes/rules) with `**related**` links between them. **Do not** default to one vault `wiki/` page per source file unless the user asks for that shape. Merge overlapping **source text** (wikitext, HTML, forum posts, etc.) about the same subject onto **one entity (or concept) page**, with multiple `raw/` paths listed under `**sources`** / `**## Sources**`.

**MediaWiki wikitext** (e.g. `*.mediawiki` exports): Source wikitext uses MediaWiki links `[[Like This]]`, templates, and categories. Vault pages use **Obsidian** links. When you summarize, **choose clear vault titles** and map confusing or duplicate source titles into those names; note ambiguity in prose or under `**## Sources`** rather than copying the external wiki’s structure blindly. Pages that are only `#REDIRECT` in the export often should **not** become separate vault pages; fold them into the target entity’s narrative and cite the redirect file if it matters.

**Legacy static HTML** (e.g. **`raw/donovansvgap/**`): Files are **not** wikitext—expect **table-heavy layout**, repeated nav/boilerplate, and old template chrome. **Cite `body.html`** for substantive claims; use **`page.meta.yaml`** when **`source_url`** or fetch metadata matters. Prefer **editorial content** over navigation chrome when synthesizing; follow the same **entity-centric** merging as other corpora. When this mirror disagrees with **Planets.nu** / Tim-era docs, resolve under **`## Sources`** / **`### Provenance and resolution`** (see **Source errors vs authoritative vault truth**).
